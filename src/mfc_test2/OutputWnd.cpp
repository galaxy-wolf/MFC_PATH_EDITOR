...
int COutputWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (/*!m_wndOutputBuild.Create(dwStyle, rectDummy, &m_wndTabs, 2) ||
		...
		!m_wndOutputInfo.Create(dwStyle, rectDummy, &m_wndTabs, 2))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
	...
	ASSERT(bNameValid);
	m_wndTabs.AddTab(&m_wndOutputInfo, strTabName, (UINT)2);
...
void COutputWnd::AdjustHorzScroll(CListBox& wndListBox)
{
	...
	for (int i = 0; i < wndListBox.GetCount(); i ++)
	{
		...
		cxExtentMax = std::fmax(cxExtentMax, dc.GetTextExtent(strItem).cx);
	}
...
void COutputWnd::ShowInfo(std::string Info)
{
	static CString s;
	s = Info.c_str();
	m_wndOutputInfo.AddString(s);
	m_wndOutputInfo.SetCurSel(m_wndOutputInfo.GetCount()-1);

}
void COutputWnd::ClearInfo()
{
	m_wndOutputInfo.ResetContent();
}

LRESULT COutputWnd::OnShowInfo(WPARAM wParm, LPARAM lParam)
{
	printf ("show info\n");
	ShowInfo("hello");
	return TRUE;
