...
bool CLoad3DS::Import3DS(t3DModel *pModel, char *strFileName, float * m_frame)
{
	...
	if(!m_FilePointer) 
	{
		//s.Format(_T("Unable to find the file: %s!"), strFileName);
		sprintf(strMessage, "Unable to find the file: %s!", strFileName);
		CString s(strMessage);
	...
	{	//s.Format(_T("Unable to load PRIMARY chuck from file: %s!"), strFileName);
...
void CLoad3DS::ReadVertices(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	for(int i = 0; i < pObject->numOfVerts; i++)
	{
		...
		// compare with m_max and m_min to get new m_max and m_min
		if (pObject->pVerts[i].x > frame[0])
			frame[0] = pObject->pVerts[i].x;
		else if (pObject->pVerts[i].x < frame[3])
			frame[3] = pObject->pVerts[i].x;
		if (pObject->pVerts[i].y > frame[1])
			frame[1] = pObject->pVerts[i].y;
		else if (pObject->pVerts[i].y < frame[4])
			frame[4] = pObject->pVerts[i].y;
		if (pObject->pVerts[i].z > frame[2])
			frame[2] = pObject->pVerts[i].z;
		else if (pObject->pVerts[i].z < frame[5])
			frame[5] = pObject->pVerts[i].z;
	}
}
...
void CLoad3DS::Destroy3DS( t3DModel *pModel )
{
	vector<t3DObject>::iterator it = pModel->pObject.begin();
	for (; it != pModel->pObject.end(); it ++)
	{
		delete [] it->pVerts;			
		delete [] it->pNormals;		
		delete [] it->pTexVerts;		
